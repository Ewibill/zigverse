...
const rest = await boot("zigrest.html", ["KeyB"]);          // rest ON — hold breath to wake
const plain = await boot("zigmurmuration.html", ["KeyB"]);  // rest OFF — regression
for (const r of [rest, plain]) {
  console.log("=== "+r.file+" ===");
...
await b.close();
const ok = [rest,plain].every(r => r.hard.length===0 && /fps|letterforms/i.test(r.status) && !/error|failed|missing/i.test(r.status));
console.log(ok ? "\nREST BOOT: PASS" : "\nREST BOOT: FAIL");
process.exit(ok?0:1);
