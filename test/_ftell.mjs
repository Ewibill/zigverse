...
async function boot(f){const p=await b.newPage();const e=[];p.on("pageerror",x=>e.push(x.message));p.on("console",m=>{if(m.type()==="error")e.push(m.text())});await p.goto("file:///home/claude/zigverse/"+f);await p.waitForTimeout(3000);await p.keyboard.press("KeyB");await p.waitForTimeout(400);const s=await p.evaluate(()=>(document.getElementById("status")||{}).textContent||"");await p.close();return{f,s:s.slice(0,50),h:e.filter(x=>!KNOWN.test(x))}}
const r=[]; for(const f of ["dist/Fatigue_And_Recover_v0.3.html","zigrest.html","zigmurmuration.html"]) r.push(await boot(f));
for(const x of r){console.log(x.f, JSON.stringify(x.s), "hard:", x.h.length); x.h.forEach(z=>console.log("  ",z.slice(0,140)))}
await b.close(); const ok=r.every(x=>x.h.length===0&&/fps|letterforms/i.test(x.s)); console.log(ok?"TELL BOOT: PASS":"TELL BOOT: FAIL"); process.exit(ok?0:1);
