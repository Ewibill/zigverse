/* =============================================================================
   ZigWebGPU â the Zigverse GPU vault (WebGPU compute + render backend)
   v0.1 Â· classic script Â· exposes a global `ZigWebGPU`
   ...
   What lives here is ENGINE, not species â every emergent swarm inherits it:
     Â· probe()        â the WebGPU render-probe gate (venue readiness)
     Â· init(canvas)   â device / context / depth / MSAA plumbing + resize
     Â· mat            â minimal mat4 (perspective Â· lookAt Â· multiply)
     Â· Watchdog       â rolling-fps â quality-tier stepper (LIVE mode only)
     Â· createFlock()  â THE FLOCK KERNEL:
         - spatial hash grid (fixed-capacity cells, atomic insert)
         - TOPOLOGICAL 7-nearest-neighbor boids (rank, not radius â the law
           that lets a murmuration morph without tearing)
         - impulse-wave propagation (a strike at a point â a wavefront kick)
         - neighbor agitation contagion (the social wave â spreads one
           neighbor-hop per step, far faster than any bird flies)
         - per-agent Drive-swell delay taps (the breath, read at each agent's
           own lag â the performer's swell travels the cloud as a wave)
         - banking â broadside/edge-on shimmer (the dark band, emergent)
         - double-buffered state â deterministic under export mode
   Species supply: meanings (what breath/attack/bend DO), palette, Director.
   ========================================================================== */
(function (global) {
  "use strict";
  const ZigWebGPU = global.ZigWebGPU || (global.ZigWebGPU = {});
  ZigWebGPU.VERSION = "0.36.0";   // 0.34: INTERIOR BUFF (back-face relief Ã0.35 + specular broadened/dimmed + gem glints Ã0.5 on !ff â the mesh facet seams stop catching as hard straight lines on the back; front keeps full crisp relief; byte-identical on the front face) Â· 0.33: GEM FACE (opts.gemFace â "inside" = the SEASHELL: matte material on the outside, gem nacre in the cupped interior; "outside"/"both" too; guards the gem's c=gc by front_facing; byte-identical at "both") Â· 0.32: MELODIC STRATA (View.noteBands[6] Â· view[84..107] â each EWI note blooms a band of light at its pitch-height in its pitch-class colour, fading over time; the melody written onto the body's vertical axis; driven by ZC.NoteField; zero when silent) Â· 0.31: SILHOUETTE RIM (live V.render5.x/y â a fresnel edge re-draws every letter's outline against the void, legible under any material on either face; face-corrected so the concave back outlines too; zero at render5.x=0; the reusable legibility capability all species inherit) Â· 0.30: GEM MATERIALS (opts.gem â refraction + dispersion fire + fresnel sky-reflection + facet flash + sparkle, sampling the analytic sky; byte-identical off) Â· 0.29: FABRIC UNDERSIDE (opts.backFabric â 20 textiles: weave pattern + sheen model + colour lining the concave back; byte-identical off) Â· 0.28: VELVET UNDERSIDE (opts.backVelvet â a different fabric skin on the back face: deep matte + grazing sheen; byte-identical off) Â· 0.27: MEMORY UNDERSIDE (opts.memoryBack â the back face glows with a lagging ghost of the recent phrase; the 2nd performance surface; render4.y/z; byte-identical off) Â· 0.26: CHIAROSCURO (opts.chiaro â back off ambient/fill so only the light-facing side shows, unlit â black; byte-identical at 0) Â· 0.25: WEB â connective filaments between neighbouring agents (grid-driven K-NN compute + instanced thread render; breath strings the web; byte-identical when opts.web absent) Â· 0.24: GRAIN THROUGH COLOUR â the skin's grain corrugates the spectrum too, so surface texture survives at full ink (byte-identical without material/spectrum) Â· 0.22: BOUNDARY Â· 0.23: COMPOSE â spectrum TINTS the material body (visible on pale skins; ink I/K = solidârainbow amount over the pigment) Â· 0.19: MEDIUM Â· 0.20: FORCES Â· 0.21: CURRENT (opts.boundary {shape,r,k,lo?,hi?} â the world's SHAPE: a soft cylinder/sphere that holds matter inside a volume; restoring accel before integrate, composes with all; byte-identical when absent) // 0.35: BOUNDARY AXIS â cylinder law holds along any free axis (capsule = horizontal cigar); byte-identical for axis "y" Â· 0.35.1: GYRE AXIS â current circulates around any axis (roll a horizontal cigar broadside); byte-identical for gyre axis "y" Â· 0.36: ELLIPSOID boundary (lens — squashed sphere, per-axis radii; the wide breathing disc); byte-identical for sphere/cylinder

  /* ---- probe â the gate. Green or it doesn't ship. ---------------------- */
  ZigWebGPU.probe = async function () {
    if (!global.navigator || !navigator.gpu)
      return { ok: false, reason: "WebGPU unavailable â use Chrome/Edge 113+ (or enable chrome://flags/#enable-unsafe-webgpu)" };
    let adapter = null;
    ...
    const desc = [info.vendor, info.architecture, info.device].filter(Boolean).join(" Â· ") || "adapter";
  ...
  /* ---- Watchdog â fps â quality tier (live mode ONLY; export pins max) --- */
  ...
  /* ---- init â device, context, depth+MSAA, resize ------------------------ */
  ZigWebGPU.init = async function (canvas, opts) {
    ...
    /* Storage-buffer headroom: the step kernel can reach ~10 storage buffers
       (flock 6 + flow + skin + arousal Ã2) â past the default per-stage cap of
    ...
    // node telemetry: surface every GPU validation error â a silent black screen
    // at a venue is the enemy (Venue Node Spec Â§3)
    ...
    // COPY_SRC: the frame is readable â the seed of the export-capture pipeline
    // (Output Targets doc: capture frames â ffmpeg â 4K file) and of verification.
  ...
  /* ========================================================================
     THE MEMORY GLASS â Afterimage law (v0.7 Â· 2026-07-20)
     ...
     trail pass folds it into a persistence buffer by the PHOSPHOR LAW â
        trail = max(scene, prevTrail Â· decay â Îµ)
     ...
     length is frame-rate independent. Ï â¤ 0.02 â decay 0 â byte-exact
     passthrough. Opt-in via scene.attachAfterimage(a) / flock.attachâ¦();
     worlds that never attach render EXACTLY as before. Every species â
     Halo Field, Lake, Fireflies, futures â inherits this with one call.
     ===================================================================== */
...
@fragment fn blitFs(inp: FSQ) -> @location(0) vec4f {
  ...
  ZigWebGPU.createAfterimage = function (gpu, opts) {
    ...
    const after = {
      tau: opts.tau === undefined ? 1.2 : opts.tau,   // memory time-constant, seconds (â¤0.02 = off)
      gate: opts.gate === undefined ? 0 : opts.gate,  // luminance floor for memory (0 = remember all Â· ~0.45 = only flashes)
      ...
      /* fold scene â trail, blit trail â glass. Recorded on the SAME encoder. */
      run(enc, swapView, dt) {
  ...
  /* ========================================================================
     ZIGFLOW â THE MEDIUM ITSELF (v0.8 Â· 2026-07-21)
     Until now the organisms flew through dead air: forces reached them only
     as wires (breath, waves, neighbors). ZigFlow gives the WORLD a body â a
     coarse 3-D velocity grid every flock in a scene shares:
       Â· ambient weather: analytic curl noise (divergence-free by
         construction â winds swirl, never drain or pile up), breath-scaled
       Â· wavefronts STIR it: a gong strike leaves rotating currents in the
         air that outlive the visible front (the medium remembers)
       Â· the avatar wakes it: the pilot's passage drags the air behind her
       Â· it forgets: exponential damping, Ï â 1/damp seconds
     ...
     exact golden kernel. Deterministic â no RNG anywhere.
     ===================================================================== */
  const FLOW_WGSL = `
struct FU {
  dim: vec4f,                   // nx, ny, nz, cell size
  org: vec4f,                   // grid origin xyz, w dt
  par: vec4f,                   // x damp/s Â· y ambient gain Â· z time Â· w flock coupling (unused here)
  wav: vec4f,                   // x waveSpeed Â· y waveWidth Â· z waveLife Â· w breath
  imp: array<vec4f, 8>,         // xyz origin Â· w start time (<0 inactive)
  istr: array<vec4f, 8>,        // x strength ('meta' is WGSL-reserved)
  stir: vec4f,                  // avatar stir point xyz Â· w on/off
  stirV: vec4f,                 // avatar velocity xyz Â· w radius
};
@group(0) @binding(0) var<uniform> F: FU;
@group(0) @binding(1) var<storage, read_write> flow: array<vec4f>;

/* curl of a sinusoidal vector potential â divergence-free wind, proven in
...
fn stirFlow(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  /* the pilot's wake â flying through the air drags it along */
  if (F.stir.w > 0.5) {
    let dd = p - F.stir.xyz;
  ...
  ZigWebGPU.createFlow = function (gpu, opts) {
    ...
    return {
      ...
      damp: opts.damp === undefined ? 0.35 : opts.damp,      // the air's forgetting rate (Ï â 2.9 s)
  ...
  /* ========================================================================
     THE MEMBRANE â elastic space (v0.10 Â· Scout's law, 2026-07-22)
     ...
     on its surface (disturbances PROPAGATE â negotiations, not waves-in-
     ...
     lived (Ïâ45 s), then forgets (Ïâ220 s). The membrane is NEVER rendered.
     Flocks with opts.skin sample it and inherit its local geometry â the
     organisms reveal the field. Breath changes ELASTICITY, not shape: a
     performer changes the weather, not the geometry.
     Simulated on CPU (512 cells, trivial) â uploaded as a storage buffer;
     the pure step lives in ZigWebGPU.membraneStep for Node-side law proofs.
     ===================================================================== */
  /* pure physics â F: {u,v,mem,lap: Float32Array(nt*np), nt, np} Â· P: params */
  ZigWebGPU.membraneStep = function (F, dt, P) {
    ...
    for (let t = 0; t < nt; t++) {
      for (let p = 0; p < np; p++) {
        ...
        /* TOPOLOGICAL MEMORY acts on CURVATURE too: the remembered shape
           becomes the surface's rest GEOMETRY, not just a local pull â so a
           held dent stays a dent instead of diffusing flat. Steady state:
           u â memWÂ·mem, exactly the lived shape, slowly forgotten. */
    ...
    for (let i = 0; i < nt * np; i++) {
      ...
      mem[i] *= Math.exp(-dt / P.tauForget);                   // â¦and slowly forgets
    }
  };
  ...
  ZigWebGPU.createMembrane = function (gpu, opts) {
    ...
    return {
      ...
      elastic: 0,                    // 0 = rigid (silence) Â· 1 = willing (full breath)
      attractW: 2.6, alignW: 1.9, stressGain: 0.55,
      sigma: 0, depth: 0,            // HUD telemetry: current deformation Â· memory depth
  ...
  /* ========================================================================
     SCENE â a shared world many species draw into.
     ...
     engine's "installation" primitive â every layered piece inherits it.
...
@fragment fn skyFs(inp: SkyOut) -> @location(0) vec4f {
  ...
  ZigWebGPU.createScene = function (gpu, opts) {
    ...
    return {
      ...
      frame(viewArr, states) {
        ...
        for (let i = 0; i < this.species.length; i++) this.species[i].recordInto(rp);   // shared depth â correct compositing
        rp.end();
  ...
  ZigWebGPU.createFlock = function (gpu, opts) {
    ...
    /* THE WARDROBE (v0.9): opts.mesh may be ONE mesh or an ARRAY of meshes.
       ...
       each agent wears THIS FRAME â the flock keeps its life (positions,
       velocities, phases, memory) and changes its body. */
    const MESHES = Array.isArray(opts.mesh) ? opts.mesh : (opts.mesh ? [opts.mesh] : null);
    const MESH = MESHES ? MESHES[0] : null;   // ZigMesh output â shard agents (render mode 2)
    const WARD = MESHES && MESHES.length > 1;
    const FORMFIELD = (WARD && opts.formField) ? opts.formField : null;   // FORM FIELD: a per-agent signal picks the letter from the rack â form expresses state ("biome" hash Â· "age" Â· "energy"). Requires a wardrobe rack.
    const SPECTRUM = !!opts.spectrum;   // ZIGSPECTRUM: map the thin-film hue ALONG the letter (u: baseâtip) instead of scattering it by per-agent random. render3.x rotates the wheel Â· render3.y spans how much of it runs baseâtip. Reusable optics; byte-identical when off.
    const REVEAL = (WARD && opts.reveal) ? (+opts.reveal || 1) : 0;   // REVEAL WINDOW: each blade renders only a baseâfrontier FRAGMENT of its letter â a per-agent amount (identity) that UNFURLS with motion/breath. Reuses the tail-collapse. The field becomes a drift of letter-fragments, never 100% of a glyph. Needs a wardrobe rack; byte-identical when 0.
    const TRANSMIT = opts.transmit ? (+opts.transmit || 1.6) : 0;   // OPTICS Â· LUMINESCENCE: backlit transmission â a thin petal caught between eye and moon GLOWS FROM WITHIN (light passes through), carrying its own colour out. VS writes the forward-scatter lobe into the shard's unused depth channel; FS adds the inner glow. Number = gain; byte-identical when 0.
    const SHEEN = !!opts.sheen;   // OPTICS Â· ANISOTROPIC SHEEN: replace the round half-vector glint with a Kajiya-Kay highlight that runs ACROSS the blade and sweeps ALONG its length axis (fwd) as it twists â silk / petal-grain, not a plastic dot. Rides the existing glint channel; byte-identical off.
    ...
    /* MICRO-MEMBRANE (v0.10.1): letters whose DNA carries `breathe` swell
       and dimple along their normals on their OWN clocks â faster and
    ...
    const PHASE = opts.phase || null; // ZigPhase law â per-agent blink oscillators (Fireflies build)
    const FLOW = opts.flow || null;   // ZigFlow â this flock rides the world's shared currents
    /* THE BIOME (v0.9.1 Â· Scout's Orchard): opts.biome = { notes: [{dark,
       light}, â¦] } bakes a family of BORN colors. Every agent draws ONE note
       at birth (deterministic hash) â no gradients, no rainbow: individual
    ...
    const WEB = opts.web ? { k: Math.max(1, Math.min(8, (opts.web.k | 0) || 3)), radius: +opts.web.radius || 16, width: +opts.web.width || 0.5 } : null;   // WEB LAW (v0.25): connective filaments between neighbouring agents â a compute pass reads the flock's own spatial grid for each agent's K nearest, then draws a thin camera-facing thread to each. Living connective tissue any species inherits; byte-identical when absent.
    /* CHIAROSCURO (v0.26) is now a LIVE uniform (View.render4.x = view[76]) â the species
       writes it each frame and a key dials it. No baked opt needed; 0 = soft full lighting. */
    const MEMBACK = !!opts.memoryBack;   // MEMORY UNDERSIDE (v0.27): the back face glows with a lagging ghost of the recent phrase (render4.y=hue, render4.z=glow). The 2nd performance surface â front=now, back=recent past. Byte-identical when off.
    const GEM = opts.gem || null;   // GEM MATERIAL (v0.30): the shard is a cut stone â fresnel rim, sky reflection, refraction bent through the body + tinted by the gem colour, DISPERSION (RÂ·GÂ·B split = fire), facet flash, sparkle. Samples the analytic sky (gemSky). Needs a front material (Nr, hvM). Byte-identical when absent.
    /* GEM FACE (v0.33): which face wears the stone â "both" (whole shard, the classic look) Â·
       "inside" (the SEASHELL: matte material outside, gem nacre in the cupped interior) Â·
    ...
    const gemBlock = (G, cond) => {
      ...
      return `${cond || ""}{
      /* GEM: refraction bent through the stone (3 iors â RÂ·GÂ·B fire) blended with a fresnel
      ...
      gc = gc + vec3f(1.0, 1.0, 0.98) * pow(max(dot(gN, gH), 0.0), 240.0) * ${f(G.facet)} * 1.6 * gBuff;   // facet flash â sharp white glint (softened on the interior)
      gc = gc + vec3f(1.0) * step(0.985, hvM(floor(vec2f(inp.obj.x, inp.dv) * 90.0))) * (0.4 + 0.6 * fres) * ${f(G.spark)} * 2.6 * gBuff;   // sparkle â RARE + bright, not static (softened on the interior)
      c = gc;
    }`;
    };
    const BACKFAB = opts.backFabric || null;   // FABRIC UNDERSIDE (v0.29): the back face is lined in a textile â a WEAVE (micro-pattern normal) + a SHEEN model (retro/spec/metal/matte) + deep base + colour. 20 fabrics in ZigCore.Fabrics. A different SKIN within; needs a front material (snw). Byte-identical when absent.
    ...
    const fabricBlock = (F) => {
      ...
      /* FABRIC UNDERSIDE (${F.weave}/${F.sheen}): weave tilts the back normal, the sheen model
         ...
         control flow â so compute it OUTSIDE the (!ff) branch (runs for all shard fragments),
    ...
    const MEDIUM = opts.medium || null;  // ENVIRONMENT Â· MEDIUM: the density/viscosity of the world the matter moves through â {drag, vmax}. Thin air â¶ resistant water â¶ thick honey. Scales the step-kernel drag + speed cap; byte-identical when absent. (Phase 2, pillar 1.)
    const FORCES = opts.forces || null;  // ENVIRONMENT Â· FORCES: gravity / buoyancy â {g (down<0 / up>0), floor|ceil, damp}. Does the matter SINK & settle, FLOAT & gather, or hang SUSPENDED? Adds vertical accel before the integrate (composes with MEDIUM drag). Byte-identical when absent. (Phase 2, pillar 2.)
    const CURRENT = opts.current || null; // ENVIRONMENT Â· CURRENT: the world's flow the matter RIDES â {d (drift vector), gyre (rotation around the anchor's vertical axis)}. drift = a tethered stream/lean Â· gyre = a whirlpool Â· both = a swirling eddy. Adds accel before integrate (composes with MEDIUM + FORCES). Byte-identical when absent. (Phase 2, pillar 3.)
    const STAGE = opts.stage || null;    // EXPERIENCE Â· STAGE (the vitrine): a floor with a soft pool of light beneath the organism â {x,y,z (pool center on the floor), r (plane half-size), pool (light radius), color[3], gain}. The specimen rests in a lit space in a dark room; the viewer becomes a voyeur looking IN. Byte-identical when absent.
    const BOUNDARY = opts.boundary || null; // ENVIRONMENT Â· BOUNDARY: the world's SHAPE â {shape "cylinder"|"sphere", r, k, lo?, hi?}. A soft surface that HOLDS matter inside a volume (bowl Â· chimney Â· vessel): where forces pull and currents push, a boundary contains. Restoring accel before integrate (composes with all); a world can't drift out of frame. Byte-identical when absent. (Phase 2, pillar 4.)
    const SKIN = opts.skin || null;   // MEMBRANE: this flock inherits the elastic field's local geometry
    const REST = opts.rest !== undefined && opts.rest !== false && opts.rest !== null;   // ZIGLIFE: per-agent arousal â rest/wake (breath wakes, silence sleeps). The individual-behavior substrate (arousalÂ·fatigueÂ·ageÂ·spare)
    const SEEK = opts.seek || null;   // ZIGSEEK: world attractor + repulsor â agents seek toward / avoid away (the base for foraging, attach, flee)
    const ATTACH = !!opts.attach;     // ZIGATTACH: bind-in-place / release â agents freeze into a held pose (bond channel of the life buffer)
    const FATIGUE = !!opts.fatigue;   // ZIGMETABOLISM: energy drains with effort, refills at rest / calm breath / huddle â stamina (energy channel of the life buffer)
    const AGING = !!opts.aging;       // ZIGAGE: a slow per-agent lifespan clock â born â prime â old â renew; generational turnover (age channel of the life buffer)
    const LIFE = REST || ATTACH || FATIGUE || AGING;   // the shared per-agent life buffer exists when ANY life behavior is on
    const REST_INIT = (typeof opts.rest === "number") ? Math.max(0, Math.min(1, opts.rest)) : 1.0;   // starting arousal (1 awake Â· low = starts asleep)
...
struct Sim {
  ...
  anchor: vec4f,                // xyz roost anchor Â· w lift height
  refpt: vec4f,                 // xyz delay reference (last strike) Â· w maxDelay
  wind: vec4f,                  // xyz wind Â· w ali_w
  knobsA: vec4f,                // x contagion Â· y sepRadius Â· z vmin Â· w vmaxBase
  knobsB: vec4f,                // x vmaxAgit Â· y waveKick Â· z bankGain Â· w churn
  impulses: array<vec4f, 8>,    // xyz origin Â· w start time (<0 inactive)
  imp_meta: array<vec4f, 8>,    // x strength
  taps: array<vec4f, 16>,       // 64 Drive taps, 0..maxDelay
  wanderers: array<vec4f, 4>,   // x birdIndex Â· yzw steer target (turnover)
  wmeta: array<vec4f, 4>,       // x steer strength (<=0 inactive)
  modes: vec4f,                 // x medium (0=air/gravity Â· 1=water/neutral) Â· y K Â· z tempo Â· w ignite (ZigPhase)
  pace: vec4f,                  // x performer phase Â· y pull (Pacemaker) Â· z waveLife s (0=golden 3.5) Â· w VOICE (1=dancer Â· 0=resonator: no light/agitation of its own, rings only when struck)
  avatarA: vec4f,               // AVATAR law: x agent index (<0 = off) Â· yzw steer target
  avatarB: vec4f,               // x steer strength Â· y flash-now (note-on frame) Â· z agit burn Â· w charisma
  morph: vec4f,                 // WARDROBE: x letter index Â· y letterB Â· z per-agent mix toward B (0..1) Â· w free
};
const EXT: f32 = ${EXT}.0;
...
fn step(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  /* ---- topological K-nearest (rank, not radius â THE LAW) ---- */
  var nd: array<f32, K>;
  var ni: array<u32, K>;
  ...
  /* ---- contagion: agitation spreads one neighbor-hop per step ----
     (this IS the social wave â it outruns any bird)                  */
  agit = max(agit * exp(-1.6 * U.dt), nAgit * U.knobsA.x);

  /* ---- impulse wavefronts (the falcon strike Â· the thrown stone) ----
     WLIFE: species-set wave lifetime (pace.z). Default reproduces the golden
     3.5s / 0.7 decay EXACTLY (2.45/3.5 = 0.7) â water remembers longer. */
  ...
  for (var m = 0u; m < 8u; m++) {
    ...
    if (f > 0.003) {
      /* per-impulse KICK SCALE (imp_meta.y): a falcon strike shoves (1.0); a
         NOTE-IMPULSE flares the same but barely pushes (~0.2) â a nerve pulse
         through the tissue, not a blow. Flare (agit) is untouched by the scale. */
      let kick = U.knobsB.y * U.imp_meta[m].y;
      if (surface > 0.5) {
        /* SURFACE: the front is a CREST â it lifts the skin as it passes,
        ...
        accel += away * f * kick;
        // AIR: the band dives as it banks (collapse). WATER: pure radial vortex â the
  ...
  /* ---- global shepherding (anchor Â· altitude Â· wind Â· lean) ---- */
  let anchor = U.anchor.xyz;
  var toA = anchor - p; toA.y = 0.0;
  let dA = length(toA);
  /* SURFACE: a lake is WIDE â the homeward pull drops to a whisper so the
  ...
  /* SURFACE (medium 2): the SKIN â spring to the lake level with damping,
  ...
  // WATER: neutral buoyancy â symmetric gentle centering, no roost, no pour.
  ...
  /* ---- turnover: a few anonymous birds quietly leave & rejoin ----
     ...
     so it peels away calmly (never a focal event â leaderless preserved).
  ...
  /* ---- AVATAR: the performer embodied (idx < 0 = law dormant) ----
     Steered like a wanderer but NEVER calmed â the avatar BURNS, and its
  ...
  /* ---- banking â the shimmer. Lateral accel rolls the bird; a rolled
         bird shows its BROADSIDE â the dark band, purely emergent. ---- */
...
struct View {
  ...
  render: vec4f,      // x renderMode(0=bird,1=fish) Â· y swimHz Â· z swimAmp Â· w swimWave
  render2: vec4f,     // x flashGain Â· y godRays Â· z caustics Â· w waterDepthShift
  render3: vec4f,     // x surfaceY Â· y depthRange Â· (zw free)  â underwater light
  render4: vec4f,     // x = CHIAROSCURO amount (live, view[76]) Â· y memHue Â· z memGlow Â· w memBack
  render5: vec4f,     // x = RIM strength (live, view[80]) Â· y = RIM sharpness (view[81]) Â· zw free
  noteBands: array<vec4f, 6>,   // MELODIC STRATA (view[84..107]): each note = (worldY, hue, energy, 0) â a band of light at the note's pitch-height, its pitch-class colour, fading over time
};
@group(0) @binding(0) var<uniform> V: View;
...
/* MARINE SNOW â faint particulate sinking through the column; the "we're
...
fn waterColor(dir: vec3f) -> vec3f {
  ...
  var col = mix(mid, deep, smoothstep(0.0, -0.75, y));   // looking down â the deep
  col = mix(col, surf, smoothstep(0.05, 0.85, y));       // looking up â the surface
  let sd = normalize(V.sunDir.xyz);
  let s = max(dot(dir, sd), 0.0);
  // Snell's window â the bright disc of the world compressed straight overhead
  let snell = smoothstep(0.82, 0.995, y);
  col = mix(col, surf * 1.5 + V.sunCol.rgb * 0.35, snell);
  col += V.sunCol.rgb * pow(s, 90.0) * 0.6 * V.sunDir.w;
  // GOD RAYS â shafts raking down from the surface, broken by an angular ripple
  let ang = atan2(dir.x, dir.z);
  let ripple = 0.55 + 0.45 * sin(ang * 16.0 + V.camPos.w * 0.35) * sin(ang * 6.3 - V.camPos.w * 0.21);
...
/* ---- caustics â the dancing net of light (surface ripples as lenses) ---- */
...
/* ---------------- starlings ----------------
   A starling, not a delta-wing: pointed head Â· chunky torso Â· short fan
   tail Â· swept POINTED wings that hinge at the shoulder. 7 triangles.   */
var<private> STARLING: array<vec3f, 21> = array<vec3f, 21>(
  // head (beak tip â shoulders)
  ...
  // wings â swept, pointed, rooted at the shoulder
...
struct BirdOut {
  ...
  @location(0) shade: f32,     // bird: broadside-ness Â· fish: specular flash
  @location(1) fog: f32,
  @location(2) agit: f32,
  @location(3) dv: f32,        // fish dorsoventral (âbelly..+back) for countershading
...
fn birdVs(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> BirdOut {
  ...
  let t = V.camPos.w;
  let agit = P.w;
  let fish = V.render.x;                 // 0 = bird Â· 1 = fish
  ...
  if (fish > 0.5) {
    /* ---- FISH: vertical-lens body, swimming undulation, mirror flank --
       ...
       sweeps side-to-side (undulation along sideR). Per-fish size jitter.
         render.y = swimHz Â· render.z = swimAmp Â· render.w = swimWave     */
    ...
    // (Undulatory Swimming, SMU). The '+ t' term makes the crest move noseâtail;
    // the smooth envelope grows the amplitude toward the tail â head barely moves.
    ...
    o.dv = clamp(l.y * 3.3, -1.0, 1.0);   // âbelly .. +back
    o.tone = 0.86 + 0.28 * h2;
    o.obj = l;                            // object-space position for the eye
  } else {
    /* ---- BIRD: flap-glide (unchanged) --------------------------------
         skyTop.w = flapHz Â· skyMid.w = flapAmp Â· horizon.w = glideMix    */
    let l = STARLING[vi];
    ...
    let reach = max(abs(l.z) - 0.25, 0.0);          // 0 on body & tail â wings only
  ...
  if (fish > 0.5) {
    ...
    // IRIDESCENCE PHASE â thin-film sheen grows toward grazing view of the
  ...
  o.agit = P.w;
  o.wpos = wp;
  o.depth = clamp((V.render3.x - wp.y) / max(V.render3.y, 1.0), 0.0, 1.0);   // surfaceâdeep
...
fn birdFs(inp: BirdOut) -> @location(0) vec4f {
  ...
  if (V.render.x > 0.5) {
    /* ---- FISH: countershading + the SILVER FLASH WAVE ----------------
       COUNTERSHADING: dark dorsal â silver flank â pale ventral (a fish is
       ...
       flank body Â· birdLight = silver Â· sunCol tints the caught light.       */
    ...
    // IRIDESCENCE â thin-film structural colour: a shifting blue/green/violet/
    ...
    // MOVES â a gentle base breathing glow, punched up by agitation, so the
    ...
    // DEPTH-SHIFT: fish fade toward the water colour at their own depth â
    ...
    // EYE â a dark spot on each flank near the head, LOD-faded so it only
    // resolves up close (at flock distance the ball stays one organism).
    // camFwd.w = eyeGain. object eye centre â (0.52, 0.09) on the flank.
    ...
    // broadside = dark silhouette Â· edge-on = catches the dusk light
...
/* ---------------- DEBRIS â suspended particulate that reads the WAVE ----------
...
@fragment fn debrisFs(inp: DOut) -> @location(0) vec4f {
  ...
  if (inp.kind < 0.5) {
    // BUBBLE â a rising ring: bright silver rim, translucent centre, catchlight
    ...
    // PLANKTON â a tiny crisp bright speck (and faint cold glow at night)
    ...
    // MARINE SNOW â a small soft irregular tan-white fleck, slow-sinking
    ...
    // FISH SCALE â a guanine flake torn loose by a strike; silver with a thin-film
  ...
  return vec4f(col, clamp(a, 0.0, 1.0));
    ...
    /* ZIGLIFE per-agent state (ping-ponged like pos/vel): life = (arousal,
       fatigue, age, spare). Only allocated when opts.rest â arousal starts
    ...
    if (LIFE) {
      ...
      for (let n = 0; n < MAX; n++) { linit[n * 4] = REST_INIT; linit[n * 4 + 1] = 1.0; linit[n * 4 + 2] = (n * 0.6180339887) % 1; }   // .x arousal Â· .y energy Â· .z age (golden-ratio spread â mixed generations from the start) Â· .w bond 0
      device.queue.writeBuffer(lifeA, 0, linit); device.queue.writeBuffer(lifeB, 0, linit);
    }
    /* ZIGSEEK: a tiny uniform of world targets â seek (attractor) + avoid
       (repulsor) + cfg(reach). Its own buffer (not the packed Sim) â the Sim
    ...
    /* ZIGATTACH: a one-value uniform â the global attach signal (0 free â 1
    ...
    const VIEWF = 108;                                   // floats in View (+ render..render5 + noteBands[6]) â render4.x chiaroscuro (v0.26) Â· render5.x rim (v0.31) Â· view[84..107] melodic strata (v0.32)
    ...
    /* ---- ZigMesh integration (additive Â· promoted during the Fireflies/
       ...
       the source below is exactly the golden string â Murmuration and School
    ...
    if (MESH) {
      ...
      /* FORM FIELD driver â the per-agent signal (0..1) that picks the letter
         from the rack. "biome" = a born hash (a fixed random form each â a mixed
         field); "age"/"energy" read the life buffer so FORM EXPRESSES STATE
         (youngâold walks the rack; the exhausted collapse to later forms). */
      let FORM_SIG = "0.0";
      ...
      const SHARD_VS = `
  if (V.render.x > 1.5) {
    /* SHARD (ZigMesh letterform) â roll-tilted like fish; x=length Â·
       y=camber-normal Â· z=lateral. Early return: fully self-shaded. */
    let cb = cos(bank); let sb = sin(bank);
    ...
    /* mark gain (render2.y): 0 = truly incognito Â· ~1 = subtle ember Â·
       ~3 = BEACON (study mode â the lead petal cannot be missed) */
    if (isAvatar) { ssize *= 1.0 + 0.32 * min(V.render2.y, 3.0); }
    ${WARD ? `/* WARDROBE / FORM FIELD â which letter this agent wears.
       Smaller letters clamp tail indices â whole triangles collapse to zero area. */
    ${FORMFIELD ? `/* FORM FIELD (${FORMFIELD}): a per-agent signal picks the letter from the rack â form expresses state. */
    let fsig = ${FORM_SIG};
    ...
    ${REVEAL ? `/* REVEAL WINDOW (the blend): render only a baseâfrontier FRAGMENT â a
       per-agent amount (its identity, rvHash) that UNFURLS with motion/breath (P.w =
       agit). Reuses the tail-collapse: verts past the frontier pile on it â zero area,
       no spikes. Rest = scattered fragments Â· play = the letters draw themselves. */
    ...
    /* low render.w = a per-agent DRIFT of fragments (each unfurls its own amount);
       ...
       overrides the per-agent scatter once reveal is pushed up (5/6 Â· N-preview). */
    let frag = clamp(V.render.w * (0.42 + 0.9 * rvHash) + 0.75 * P.w, 0.12, 1.0);
    let whole = clamp((V.render.w - 0.9) / 0.7, 0.0, 1.0);   // 0 at render.wâ¤0.9 â 1 (all verts) at render.wâ¥1.6
    ...
    /* FLUID SMEAR (render2.w gain): petals stretch along their motion â
    ...
    ${BREATHE > 0 ? `/* MICRO-MEMBRANE: this letter is a film with its own pulse â
    ...
    o.dv = aux.x;                                           // edge (â1..1)
    /* GLINT: true half-vector sparkle â lights only where tilt bisects
       moonâeye. On a wavy field this draws the MOONPATH, and every passing
    ...
    ${SHEEN ? `/* ANISOTROPIC SHEEN (Kajiya-Kay): the highlight runs ACROSS the blade and
       sweeps ALONG its length as it twists â silk / petal-grain, not a round dot.
       T = fwd (the blade's length axis); the band lives where H â T. */
    let sheenTH = dot(fwd, hvec);
    var glint = pow(sqrt(max(1.0 - sheenTH * sheenTH, 0.0)), 44.0);` : `var glint = pow(max(dot(sn, hvec), 0.0), 26.0);   // broad lobe â twisted petals still catch it`}
    /* THE LANE: concentrate the sparkle into the corridor between eye and
       moon azimuth â a moonPATH, not an even frost */
    ...
    o.obj = vec3f(aux.y, glint, select(0.0, V.render2.y, isAvatar));   // u Â· glint Â· ember
    o.tone = 0.85 + 0.30 * h2;
    let sd = distance(V.camPos.xyz, swp);
    o.fog = 1.0 - exp(-sd * V.birdLight.w);
    o.agit = P.w;
    o.wpos = swp;
    ${TRANSMIT ? `/* LUMINESCENCE: forward-scatter lobe â strong where the moon sits
...
`;
      ...
      /* CHIAROSCURO shading builder (v0.26): the ambient + shadow-floor scale by (1 â V.render4.x),
         LIVE via the new render4.x slot. At render4.x = 0 this is arithmetically the original
         soft lighting (Ã1.0), so every other demo is unchanged; toward 1 only the light-facing
      ...
      const SHARD_FS = `
  if (V.render.x > 1.5) {
    /* SHARD: two-tone dome/hollow â the flip IS the letter. birdDark = moss
       dome Â· birdLight = bone hollow Â· mid-turn iridescence rides agitation,
...
${(SPECTRUM && MAT) ? `    var matGrain = 0.5;                                    // MATERIAL grain height (set by the material law) â lets texture read THROUGH the spectrum
` : ``}    var c = ${SHADE("inp.shade")};
    ...
    /* the rainbow: thin-film hue Â· gain = V.render3.z (species-controlled ink,
       same dial as fish iridescence). Per-agent phase (tone) + along-length
       drift spread the SPECTRUM across the field instead of one hue Ã6000;
       agitation nearly doubles it, so the wave rolls through as color. */
    ${SPECTRUM ? `/* ZIGSPECTRUM: thin-film hue mapped ALONG the letter (spU 0=base â 1=tip).
       render3.x rotates which colors land where Â· render3.y spans how much of the
       wheel runs baseâtip Â· a whisper of tone breaks the decal. THE LIGHTING SHOWS
       PORTIONS: the color rides pow(iri) â only the grazing/turning parts of each
    ...
    let spU = inp.obj.x;
    /* SPREAD (V.render.z Â· 7/8 keys): per-agent hue scatter. 0 = one coherent band the whole
       field shares (Q sweeps it through a single dominant hue) â up = each shard takes its own
    ...
    /* COMPOSE (v0.23): the spectrum TINTS the material body â so it reads on PALE skins too,
       not only dark ones â plus a glow. Scaled by ink (render3.z = I/K) Ã grazing Ã agit:
       ink 0 = pure material solid Â· ink up = the rainbow blooms over the pigment. */
    ...
    /* SHADOW COMPLEMENT (V.render.y Â· 9/0 keys): the dark side takes the COMPLEMENT of the
       current SPECTRUM hue â always a real colour even on a grey/pale skin (gold wheel â blue
       shadow), and it follows Q around the wheel. Value-preserving: (max+min)âhue. Weighted to
       the shadow side (1âshade) so lit areas keep the primary. 0 = classic dark. */
    ...
    ${(SPECTRUM && MAT) ? `/* GRAIN THROUGH COLOUR (v0.24): the skin's grain corrugates the spectrum too, so the
       surface texture survives even at full ink â texture is a permanent property of the
    ...
    ${TRANSMIT ? `/* LUMINESCENCE (backlit transmission): a thin petal between you and
       the moon stops reflecting and GLOWS FROM WITHIN â moonlight passes through it,
    ...
    /* the avatar's subtle ember â a warmth its kin don't carry (obj.z gain) */
    ...
    /* THE RESONATOR (S.pace.w voice, 2026-07-20): a voice-0 stratum is the
       instrument BODY, not a second dancer. It holds NO light of its own â
       moon, moonpath, rainbow, everything is gated to zero â and glows only
    ...
    ${MEMBACK ? `if (!ff && V.render4.w > 0.001) {
      /* MEMORY UNDERSIDE (v0.27): the back doesn't show the present colour â it glows with
         a lingering GHOST of the recent phrase. render4.y = the remembered hue (a slow lag
         of what you've played) Â· render4.z = how strongly it still burns (rises as you play,
         fades over seconds of silence) Â· render4.w = live on/off amount (Shift+M). The front
         is where you ARE; the back is where you've BEEN â the turning field flashes it. */
      let mh = V.render4.y;
      ...
      /* TINT the already-TEXTURED back (grain/relief/specular survive) toward the memory
         hue, preserving its light-and-dark so the surface still reads â then a soft inner
    ...
    /* SILHOUETTE RIM (v0.31 Â· live V.render5.x): a fresnel edge RE-DRAWS each
       letter's outline against the void â legible under ANY material (gem,
       fabric, bare skin) and on EITHER face. It depends on NOTHING but wpos â
       ...
       flow â legal. abs(dot) makes it face-independent: the silhouette is where
       the surface turns perpendicular to the eye, front OR concave back.
       render5.y = sharpness (thinâwide edge). Zero at render5.x = 0. */
    ...
    /* MELODIC STRATA (v0.32): the MELODY written onto the body's vertical axis â as LIGHT,
       ...
       it BRIGHTENS â the body's OWN colour lit up, plus a neutral grazing glow â scaled by the
...
`;
      RENDER_SRC = RENDER_SRC
        ...
        .replace("@group(0) @binding(2) var<storage, read> vel: array<vec4f>;",
                 ...
                 /* VOICE: the shard FS reads the flock's own Sim uniform (same
                    buffer as compute) through a thin mirror â pace lands at
      ...
      /* BIOME splices â per-agent born color + companion metal on the edge */
      if (BIOME) {
        ...
        RENDER_SRC = RENDER_SRC
          .replace("struct View {",
            ...
            "struct View {")
          .replace(a1, "o.wpos = swp;\n    o.depth = f32(ii);   // BIOME: who I am â my born color rides this")
          .replace(a2,
            `/* THE ORCHARD LAW: every letter is born ONE note of the family â
    ...
    let bone = BIOME_L[bi];`)
          .replace(a3,
            `/* COMPANION METAL: the leading edge catches burnished copper â
    ...
    ` + a3);
      ...
      /* MATERIAL splice (v0.10.2 Â· normal-RELIEF): the composition profile â
         fractal grain + weathering + subsurface â in the letter's OWN uv
         ...
         bump â no precomputed tangents), so the surface CATCHES AND LOSES LIGHT
         in its own grain and resolves at every zoom. All changes live inside
         this block â byte-identical without opts.material. */
      if (MAT) {
        ...
        const sample = (t[0] === 2)                       // fiber â grain runs ALONG the letter (anisotropic)
        ...
        const helpers =
          ...
          /* FOOTPRINT-AWARE fbm: each octave fades toward its mean (0.5) once its
             period drops below the pixel footprint w â analytic mipmapping, so
    ...
    /* THE MATERIAL LAW in the field â composition WITH RELIEF, FOOTPRINT-AWARE.
       ...
       along length Â· dv = across the width. */
    ...
    if (!ff) { Ng = -Ng; }   // UNDERSIDE: flip the normal so the back's grain, relief & specular are lit correctly â texture reads on the underside as it turns to the light` : ``}
    let dpx = dpdx(inp.wpos); let dpy = dpdy(inp.wpos);
    let r1 = cross(dpy, Ng); let r2 = cross(Ng, dpx);
    let det = dot(dpx, r1);
    let idet = sign(det) / max(abs(det), 1e-6);          // det==0 â idet 0 â no perturbation
    ...
    let reliefAmt = select(MAT_RELIEF, MAT_RELIEF * 0.35, !ff);   // BUFF the interior (v0.34): soften the relief on the BACK so the mesh facet seams stop catching as hard straight lines â the cut still reads, the grid softens (front keeps its full crisp relief Â· feeds the gem too, which shares Nr)
    ...
    /* STUDIO SPECULAR (v0.26 Â· rides the light dial render4.x): the material's own
       highlight from the single light, per-pixel over the relief normal â it SWEEPS
    ...
    /* ---- ZigPhase (additive Â· the TIME law, promoted during the Fireflies
       ...
       neighbors' phases â Kuramoto over the SAME topological graph the flock
       flies on. U.modes.y = coupling K (chaos â lock) Â· U.modes.z = tempo
       bias Â· U.modes.w = ignition kick (impulse fronts yank phase toward the
...
fn phase(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  /* topological 7-NN â the same law the wings obey, now heard by the clock */
  var nd: array<f32, K>;
  var ni: array<u32, K>;
  for (var k = 0u; k < K; k++) { nd[k] = 1e12; ni[k] = 0xffffffffu; }
  let c = cellCoord(p);
  ...
  /* Kuramoto pull toward the neighbors' phases. CHARISMA: when one of your
     seven neighbors is the avatar, its voice weighs more â persuasive, not
     dictatorial. */
  ...
  var dth = om * U.modes.z;                                   // own clock Ã tempo bias
  if (cnt > 0.0) { dth += U.modes.y * pull / cnt; }           // K decides chaos â lock

  /* PACEMAKER â the performer is one more oscillator in the graph. The pull
  ...
  /* AVATAR: a note-on flashes the performer's body NOW â the rhythm enters
  ...
  phOut[i] = vec2f(th, om);
    ...
    /* Lantern render â points of additive light on the dark. Brightness is a
       PULSE of the phase (fast rise, slow decay â a real firefly's lantern,
       not a sine). birdLight.rgb = lantern color Â· birdDark.w = size.       */
...
fn lanternFs(inp: LOut) -> @location(0) vec4f {
  ...
  /* GLOW (v0.10.2): birdLight.w = per-lantern light budget. Additive light
     STACKS â 6000 overlapping lanterns burn to white unless each is dimmed.
  ...
  return vec4f(col, 1.0);
    ...
    /* VOICE splice (mesh flocks only â the golden meshless string is byte-
       identical): a voice-0 resonator drains agitation 5Ã slower, so a strike
    ...
    /* ZIGFLOW splice (flow flocks only â golden string untouched elsewhere):
...
fn flowAt(pw: vec3f) -> vec3f {
  ...
  return mix(mix(v00, v10, f.y), mix(v01, v11, f.y), f.z);
        ...
        .replace(a2,
          "/* FLOW LAW: the medium carries currents â every wing leans toward the wind */\n  accel += (flowAt(p) - v) * FL.par.w;\n  " + a2);
    }
    /* MEMBRANE splice (skin flocks only): sample the elastic field at my
       bearing â inherit its surface (soft shell spring), its motion (radial
       velocity), and its STRESS (curvature becomes light â iridescence only
...
@group(0) @binding(10) var<uniform> SK: SkinU;`)
  ...
  ` + s2);
    ...
    /* ZIGSEEK splice (seek/avoid flocks only): a world attractor + repulsor.
       ...
       smooth distance falloff (strong near, fading past its reach) â a target
       is a REGION of interest, not an infinite pull. The base for foraging,
       attachment, and predator-flight. Its own uniform (binding 13) â
    ...
    /* ---- KERNEL HOOK Â· PRE_INTEGRATE ---------------------------------------
       The one injection point in the step kernel: every accel modifier (SEEK,
       arousal, aging, fatigue, FORCES, CURRENT, â¦) prepends its block immediately
       BEFORE this line via `.replace(K_PREINT, block + K_PREINT)`. Named ONCE here
       so the anchor lives in a single place â change the kernel line, edit only
...
` + q2);
    ...
    /* ZIGLIFE SUBSTRATE (any life behavior): ONE per-agent buffer
       ...
       its own channel of `lifeS` â so rest, attach (and later fatigue/age)
...
@group(0) @binding(12) var<storage, read_write> lifeOut: array<vec4f>;`)
        .replace(l2, l2 + "\n  var lifeS = lifeIn[i];   // ZIGLIFE: (arousal, fatigue, age, bond) â modified per-behavior, written once")
        .replace(l3, "lifeOut[i] = lifeS;\n  " + l3);
    }
    /* ZIGLIFE AROUSAL (rest/wake): breath OR a passing disturbance wakes an
       organism; silence lets it drift to sleep â wake fast, sleep slow. A
...
` + r2)
    ...
    /* ZIGATTACH (bind-in-place / release): a global attach signal (AT.p.x)
       ...
       freeze/melt travels as a wave. A bonded agent holds position â forces
       ignored, motion damped, its speed floor released so it can truly stop â
...
@group(0) @binding(14) var<uniform> AT: AttachU;`)
  ...
  var attachBond = 0.0;
  {
    ...
    let thr = 0.35 + 0.30 * hh;                                   // tighter spread â a CRISP freeze with a hint of wave
    ...
    lifeS.w = bond;
    attachBond = bond;
    accel *= 1.0 - bond;                                          // bonded = ALL forces off â hold the SHAPE, don't collapse to a mound
    v *= 1.0 - bond;                                              // bonded = velocity zeroed â truly still
    agit *= 1.0 - 0.9 * bond;                                     // bonded = calm (no blaze) â reads as frozen, not active
  }
` + a2)
        .replace(a3, "let vmin = U.knobsA.z * (1.0 - 0.97 * attachBond);");
    }
    /* ZIGMETABOLISM splice (fatigue / recover Â· energy channel .y): EFFORT
       ...
       neighbors share/conserve). An exhausted agent can't drive or blaze â it
       ...
       varies. Rides the shared life buffer â NO new bindings. Byte-identical
  ...
  {
    let hs = fract(sin(f32(i) * 45.164 + 4.1) * 13733.19);     // per-agent stamina â some tire faster
    var energy = lifeS.y;                                       // var (mutable) â reassigned below
    let effort = clamp(agit, 0.0, 1.0);                         // exertion proxy â activity / excitement / chasing
    let huddle = clamp(cnt / f32(K), 0.0, 1.0);                 // dense neighbors share & conserve
    let drain = (0.22 + 0.16 * hs) * effort;                    // effort costs (staggered per agent) â steep enough to feel
    let recover = 0.035 + 0.11 * U.breath + 0.05 * huddle;      // rest floor Â· the performer's breath Â· the huddle
    energy = clamp(energy + (recover - drain) * U.dt, 0.0, 1.0);
    lifeS.y = energy;
    let spent = 1.0 - energy;
    accel *= 0.40 + 0.60 * energy;                             // exhausted = weaker drive â forced to slow and rest
    agit *= 0.35 + 0.65 * energy;                              // exhausted = can't blaze
    accel.y -= spent * 0.40 * (1.0 - min(U.modes.x, 1.0));     // the tired DROOP â an exhausted thing visibly sags (air only)
  }
` + f2);
    }
    /* ZIGAGE splice (aging Â· lifespan channel .z): a slow per-agent clock â
       born (0) â prime â old â renew (wraps to 0: a new generation in place).
       ...
       Per-agent lifespans vary. Rides the shared life buffer â NO new bindings.
  ...
  {
    let lh = fract(sin(f32(i) * 92.31 + 2.7) * 51234.9);       // per-agent lifespan â some live longer
    let ageRate = 0.010 + 0.014 * lh;                          // ~ 42..100 s lifespans, staggered
    let age = fract(lifeS.z + ageRate * U.dt);                 // 0 â 1 â renew (a new generation in place)
    lifeS.z = age;
...
` + g2);
    ...
    if (FORCES) {
      /* ---- ENVIRONMENT Â· FORCES: gravity / buoyancy (Phase 2, pillar 2) ---------
         The world pulls the matter DOWN (sink â settle into a mound), UP (float â
         gather at a ceiling), or holds it (suspend â hang weightless). A constant
      ...
      const fBlock = `  /* ---- ENVIRONMENT Â· FORCES: gravity / buoyancy ---- */
  accel.y += ${g};${settle}
  accel.y -= v.y * ${damp};
`;
    ...
    if (CURRENT) {
      /* ---- ENVIRONMENT Â· CURRENT: the world's flow (Phase 2, pillar 3) ---------
         The matter RIDES a flow the world imposes â a DRIFT it leans/streams into,
      ...
      /* GYRE AXIS: the mass circulates AROUND a chosen axis (mean-zero tangential spin). "y"
         ...
         rotisserie â a horizontal cigar keeps presenting its full width instead of swinging
      ...
      const cBlock = `  /* ---- ENVIRONMENT Â· CURRENT: the world's flow (drift + gyre) ---- */
...
`;
    ...
    if (BOUNDARY) {
      /* ---- ENVIRONMENT Â· BOUNDARY: the world's SHAPE (Phase 2, pillar 4) ---------
         A soft surface that HOLDS the matter inside a volume. Where forces pull and
         currents push, a boundary contains â so a place is somewhere the matter IS,
         ...
         off. cylinder = a radial wall (Â± floor/ceiling) â bowl / chimney; sphere = a
    ...
    if (bd > ${r}) { accel += (-brel / max(bd, 1e-3)) * (bd - ${r}) * ${k}; }`;
        ...
        // per-axis radii â a squashed sphere. Work in unit-sphere space (divide by each radius);
    ...
    if (bd > 1.0) { let bdir = normalize(vec3f(bs.x / ${rx}, bs.y / ${ry}, bs.z / ${rz})); accel -= bdir * (bd - 1.0) * ${kEff}; }`;
      } else {   // cylinder â radial wall (Â± cap) around a free AXIS. axis "y" = upright chimney
    ...
    if (brad > ${r}) { let bn = vec2f(brel.${r1}, brel.${r2}) / max(brad, 1e-3); let bpush = (brad - ${r}) * ${k}; accel.${r1} -= bn.x * bpush; accel.${r2} -= bn.y * bpush; }${vert}`;
      ...
      const bBlock = `  /* ---- ENVIRONMENT Â· BOUNDARY: the world's shape holds the matter in ---- */
  {
...
`;
    ...
    if (MEDIUM) {
      /* ---- ENVIRONMENT Â· MEDIUM: the density/viscosity of the world -------------
         The SAME organism moves differently by what it swims through â thin air
      ...
      STEP_SRC = STEP_SRC
        .replace(mDrag, "  v *= (1.0 - U.dt * " + (+MEDIUM.drag).toFixed(3) + ");   // MEDIUM: the world's viscosity (thin air â¶ thick honey)")
    ...
    /* ZIGLIFE render tell (rest worlds Â· mesh): arousal SHRINKS a sleeping
       letter (it withdraws into itself) and blooms it awake â the reusable
       "state becomes visible" channel, bound at render binding 5. Guarded â
    ...
    if (LIFE && MESH) {
      ...
      if (REST) {   // AROUSAL â withdraw: asleep = small (shrunk) Â· awake = bloomed
      ...
      if (FATIGUE || AGING) {   // the base tone carries life-state â energy (fatigue) drains the light; the age life-arc fades the young-born and the old
        ...
        if (AGING)   f += " * (0.25 + 0.75 * smoothstep(0.0, 0.12, lifeR[ii].z) * (1.0 - smoothstep(0.72, 1.0, lifeR[ii].z)))";   // born dim â prime vivid â old fades
    ...
    if (PHASE) {
      ...
      lanternPipe = device.createRenderPipeline({
        ...
        fragment: { module: lMod, entryPoint: "lanternFs", targets: [{ format: gpu.format, blend: {
          color: { srcFactor: "one", dstFactor: "one" },                 // ADDITIVE â light adds to light
    ...
    if (DMAX > 0) {
      ...
      const dkind = new Float32Array(DMAX);   // 0 bubble (rises) Â· 1 plankton (drifts) Â· 2 marine snow (sinks) Â· 3 shed scale (event)
      const dlife = new Float32Array(DMAX);   // scale slots only: 1â0 lifetime (0 = dormant, invisible)
      ...
      // Reserve the top slots of the pool for SHED SCALES â guanine flakes torn
      ...
      for (let i = 0; i < DMAX; i++) {
        ...
        else { const rt = drng(); dkind[i] = rt < 0.22 ? 0 : (rt < 0.60 ? 1 : 2); }   // ~22% bubbles Â· 38% plankton Â· 40% marine snow
      }
      const dv = new Float32Array(DMAX * 6 * 6);   // 6 verts Â· (pos3 + uv2 + bright/agit 2 â but 6 floats: pos3,uv2,bright... pack pos3,uv2,packedB1? use 6: x,y,z,u,v,B) â agit shared
      const dQuad = new Float32Array(DMAX * 6 * 8); // 6 verts Â· 8 floats (pos3, uv2, bright, agit, pad)
      ...
      debris = {
        ...
        update(state, eye) {
          ...
          // BREATH â water pressure: a swell inflates the medium. We use breath's
          // RATE OF CHANGE so a crescendo pushes the water OUT from the ball and a
          // decrescendo draws it back â the water breathes with the performer,
            ...
            for (let m = 0; m < imp.length; m++) {
              ...
              if (im.t0 !== impT0[m]) {
                ...
                for (let k = 0; k < nsc; k++) {
                  ...
                  dvel[s * 3 + 1] = el * sp * 0.4 + 1;       // some fly up, some down â then all sink
          ...
          for (let i = 0; i < DMAX; i++) {
            ...
            const kind = dkind[i];
            // SHED SCALE â event particle: sinks, flutters, and FLASHES silver as
            ...
            if (kind > 2.5) {
              ...
              dlife[i] = life - dt * 0.42;                                 // ~2.4s â no accumulating bouquets
              dpos[i * 3] = x; dpos[i * 3 + 1] = y; dpos[i * 3 + 2] = z;
              dvel[i * 3] = vx; dvel[i * 3 + 1] = vy; dvel[i * 3 + 2] = vz;
              const rot = t * dspin[i] + dphase[i];
              const face = Math.abs(Math.sin(rot));                        // 1 broadside flash Â· 0 edge-on
              const br = Math.min(1, life * 1.4) * (0.12 + 0.72 * face);   // dim between flashes â a sparkle, not a lamp
            ...
            const riseV = kind < 0.5 ? 7.0 : (kind < 1.5 ? -0.2 : -1.4);   // bubble up Â· plankton ~neutral Â· snow down
            ...
            // the breath swell â radial push from the ball, falling off with distance
            ...
            // emit a camera-facing quad â each TYPE has its own size + shape so
            // the water reads as a mix of real matter, not uniform specks:
            //   bubble  â medium, round, bright rim (the shader draws the ring)
            //   planktonâ tiny bright speck
            //   snow    â small, slightly elongated tan flake
    ...
    /* ---- MELODIC RIBBON (opt-in Â· opts.ribbon = max spine points) ----------
       A reusable noteâform drawable: the species feeds a polyline of spine points
       ...
       MSAA + resolve. The note-twin of the breathâlife binding â any signal history
...
@fragment fn ribFs(inp: ROut) -> @location(0) vec4f {
  ...
  return vec4f(inp.col.rgb, inp.col.a * f * f);   // bright core â feathered edges (a glowing streamer, not a slab)
}
` });
      const ribArr = new Float32Array((RIBMAX - 1) * 6 * 8);   // 2 tris/segment Â· 8 floats/vert (pos3 + rgba4 + edge1)
      ...
      ribbon = {
        ...
        /* build from spine points [{x,y,z,w,r,g,b,a}] using the camera eye â width
           runs â the segment and toward the eye (camera-facing). e = Â±1 across the
           width â the FS feathers the edges. Sets count for draw. */
        build(pts, eye) {
    ...
    /* ---- SMOKE / FOG (opt-in Â· opts.smoke = particle count) ----------------
       ...
       facing soft billboards drawn in the field pass (shares depth â the smoke
...
@fragment fn smFs(inp: SOut) -> @location(0) vec4f {
  ...
  return vec4f(inp.col.rgb * a, a);                            // premultiplied â additive glow
}
` });
      const smArr = new Float32Array(SMOKEMAX * 6 * 9);   // 6 verts/quad Â· 9 floats (pos3 + uv2 + rgba4)
      ...
      smoke = {
        ...
        frame(env) {
          ...
          let rx = eye[2], rz = -eye[0]; const rl = Math.hypot(rx, rz) || 1; rx /= rl; rz /= rl;   // horizontal camera-right Â· up = world Y
    ...
    /* ---- WEB (opt-in Â· opts.web) â CONNECTIVE FILAMENTS ---------------------
       Threads drawn between neighbouring agents. A compute pass reads the flock's
       OWN spatial grid â the same topology the flock flies on â to find each
       ...
       connective tissue â any species inherits it. Byte-identical when absent. */
    let web = null;
    if (WEB) {
      const WK = WEB.k;
      ...
      const webArr = new Float32Array(4);   // x gain Â· y halfWidth Â· z hue Â· w breath
      /* ENERGY CHANNEL (v0.25) â a per-agent scalar that DIFFUSES along the web
         ...
         conducts OUTWARD across the membrane as a wavefront of light â the web as
      ...
      const injArr = new Float32Array(8);   // seed: x y z strength Â· cfg: radiusÂ² decay diffuse injGain
      injArr[4] = (WEB.radius * 0.8) * (WEB.radius * 0.8);   // injection radiusÂ²
...
struct Inj { seed: vec4f, cfg: vec4f };   // seed xyz + strength Â· cfg: radiusÂ² decay diffuse injGain
...
` });
      ...
      /* thread render: instanced camera-facing quads, one per (agent Ã neighbour).
         Instance count = countÂ·K (JS-bounded), so agent index is always in range;
         a missing neighbour (0xffffffff) is pushed behind the near plane â clipped. */
...
struct View { viewProj: mat4x4f, camPos: vec4f, camRight: vec4f, camUp: vec4f, camFwd: vec4f,
  sunDir: vec4f, skyTop: vec4f, skyMid: vec4f, horizon: vec4f, ground: vec4f,
  sunCol: vec4f, birdDark: vec4f, birdLight: vec4f, render: vec4f, render2: vec4f, render3: vec4f };
struct WebU { p: vec4f };   // x gain Â· y halfWidth Â· z hue Â· w breath
@group(0) @binding(0) var<uniform> V: View;
@group(0) @binding(1) var<storage, read> pos: array<vec4f>;
...
` });
      ...
      web = {
        ...
        eBG:  [mkWebE(posB, energyA, energyB), mkWebE(posA, energyB, energyA)],   // flip 0: energy AâB Â· flip 1: BâA
        ...
        /* pour energy at a world point (a note). strength 0 = no injection this frame;
           held notes keep feeding the seed â a fountain that diffuses outward. */
    ...
    /* ---- STAGE / THE VITRINE (opt-in Â· opts.stage) --------------------------
       ...
       held mass a ground to rest on and a lit space to inhabit â so the viewer
...
@fragment fn stFs(inp: StOut) -> @location(0) vec4f {
  ...
  return vec4f(c, pool);                                    // premultiplied â additive glow on the black floor
}
` });
    ...
    /* ---- bind groups (double-buffered AâB, BâA) -------------------------- */
    ...
    // clearGrid only touches gridCount â auto layout exposes just that binding
    ...
    const mkDrawBG = (pcur, vcur, phcur, lcur) => {
      ...
      if (LIFE && MESH) entries.push({ binding: 5, resource: { buffer: lcur } });     // ZIGLIFE: state â visible (arousal shrink / energy dim)
    ...
    const flock = {
      ...
      vertsPerAgent: MESHES ? MAXV : (opts.verts || 21),   // bird = 21 Â· fish = 33 Â· shard = largest letter in the wardrobe
      seed: seedBirds,
      /* Surface seeding â petals spread uniformly across the whole membrane
      ...
      /* ZigPhase: deterministic phase + natural-frequency seeding.
         baseOmega rad/s (â0.7 Hz â 4.4) Â· spread = Â±fraction of base. */
      ...
      _packSim(state) {
        ...
        simArr.set(state.knobsA, 24);          // contagion Â· sepRadius Â· vmin Â· vmaxBase
        simArr.set(state.knobsB, 28);          // vmaxAgit Â· waveKick Â· bankGain Â· churn
        for (let m = 0; m < 8; m++) {
        ...
        simArr[192] = state.medium || 0;       // 0 air Â· 1 water
        simArr[193] = state.K || 0;            // ZigPhase: Kuramoto coupling (chaos â lock)
        ...
        /* VOICE (the resonator law, 2026-07-20): 1 = full dancer (default â
           every existing species) Â· 0 = instrument body: the flock holds no
           light and no restlessness of its own, glows only where a wave has
           just passed, and its agitation drains 5Ã slower so the ring FADES
           instead of stopping. Read by the step kernel + shard FS (mesh). */
        simArr[199] = state.voice === undefined ? 1 : state.voice;
        /* AVATAR â the performer embodied as ONE agent; influence spreads
        ...
        /* WARDROBE â which letter the flock wears this frame. letter/letterB
           index the baked wardrobe; mix = fraction of agents wearing B
           (per-agent, deterministic hash â the METAMORPHOSIS dial). */
        simArr[208] = state.letter || 0;
        simArr[209] = state.letterB || 0;
        simArr[210] = state.mix || 0;
        simArr[211] = state.drift || 0;   // BIOME temperature drift (in notes â the orchard's slow season)
        if (SEEK) {
      ...
      /* ---- SCENE MODE â attach to a shared world (multi-species) ----------
         ...
         is untouched â Murmuration keeps its proven path.                    */
      ...
      attachScene(scene) {
        ...
        const sceneEntries = (pcur, vcur, phcur, lcur) => {
          ...
          if (LIFE && MESH) e.push({ binding: 5, resource: { buffer: lcur } });   // ZIGLIFE: state â visible (arousal shrink / energy dim)
          return e;
        };
        this._sceneDraw = [
      ...
      frame(state, viewArr) {
        ...
        if (web) { rp.setPipeline(web.rPipe); rp.setBindGroup(0, web.rBG[flip]); rp.draw(6, this.count * web.k); }   // WEB: connective filaments (after the shards â occluded behind nearer blades, weaving through the body)
        ...
        /* frame readback â export capture / verification (one frame on request) */
        let cap = null;
        if (this._capResolve) {
  ...
  /* ========================================================================
     FOREST â the ROOTED archetype (the second great primitive).
     GPU-instanced anchored strands: each holdfast roots a tapering stipe that
     bends under a CURRENT vector â stiff at the base, loose at the tip. The
     sway is pure vertex work (no sim); the strand is a camera-facing ribbon so
     it always reads. Inherits the underwater light (depth-shift Â· caustics Â·
     ...
     Instance data (vec4 per strand): rootX Â· rootZ Â· height Â· phase.
     ===================================================================== */
  /* ========================================================================
     FOREST v2 â the ROOTED archetype, ARTICULATED (movement ported from
     ...
     forces â upright stiffness, current lean, travelling-swell wave (breath by
     DELAY via Drive.tap), micro-eddies, buoyancy, surge, drape, carry â each
...
@fragment fn kelpFs(inp: KOut) -> @location(0) vec4f {
  ...
  } else if (inp.kind < 2.5) {                // FLOAT (pneumatocyst) â soft
    col = F.colFloat.rgb; alpha = across * across * (0.5 + 0.3 * inp.depth);
  } else {                                    // HOLDFAST â dark root
  ...
  ZigWebGPU.createForest = function (gpu, opts) {
    ...
    for (let i = 0; i < NP; i++) {
      ...
      // few towers â deliberate framing placement (sides); many â scattered forest
      let rootX, rootZ;
    ...
    const viewBuf = device.createBuffer({ size: 108 * 4, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });   // 108: +noteBands[6] strata (v0.32) â a flock may bind this scene's view
    ...
    let vi = 0;
    const P = []; const LI = []; const WI = []; const WA = [];  // spine pts Â· light Â· width Â· real-3D width axis
    ...
    /* ribbon: if `real3d`, use the precomputed 3D twisting width axes in WA[]
       (blades read as genuine 3D straps â orbit reveals flatâedge); else the
    ...
    const forest = {
      ...
      _build(state, viewArr) {
        ...
        for (const p of plants) {
          ...
          /* stipe â articulated chain in the current's vertical plane */
          ...
          /* blades â each flows out on its own azimuth, sways + twists */
          for (const b of p.blades) {
            ...
            const perp = [-Math.sin(b.az), 0, Math.cos(b.az)];   // horizontal, â to the blade's plane
            ...
            for (let k = 0; k <= BLSEG; k++) {
              ...
              // long axis â the blade direction in its plane (cosÂ·[bx,0,bz] + sinÂ·up)
              const ca = Math.cos(ang2), sa = Math.sin(ang2);
              const lx = bx * ca, ly = sa, lz = bz * ca;                     // blade forward
              // n = normalize(cross(long, perp)) â the strap's other face axis
      ...
      computeInto(enc, state) {           // rooted â rebuild the mesh against the shared view + upload
