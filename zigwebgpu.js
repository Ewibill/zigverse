...
@fragment fn skyFs(inp: SkyOut) -> @location(0) vec4f {
  ...
  ZigWebGPU.createScene = function (gpu, opts) {
    ...
    return {
      gpu, viewBuf, species: [], _viewArr: null,
...
struct Sim {
  ...
  modes: vec4f,                 // x medium (0=air/gravity · 1=water/neutral)
};
const EXT: f32 = ${EXT}.0;
...
fn step(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  /* ---- impulse wavefronts (the falcon strike) ---- */
  for (var m = 0u; m < 8u; m++) {
    let im = U.impulses[m];
    ...
    let f = exp(-x * x / (2.0 * w * w)) * U.imp_meta[m].x * exp(-age * 0.7);
    if (f > 0.003) {
      ...
      accel.y -= f * U.knobsB.y * 0.33 * (1.0 - U.modes.x);
      agit = max(agit, f);
    }
  }
  ...
  let dA = length(toA);
  if (dA > 1.0) { accel += toA / dA * min(dA * 0.02, 1.0) * 3.0; }
  ...
  v += accel * U.dt;
  v *= (1.0 - U.dt * 0.9 * U.modes.x);   // WATER: viscous drag (fish glide to rest)
  let sp = length(v);
...
@fragment fn debrisFs(inp: DOut) -> @location(0) vec4f {
  ...
  return vec4f(col, clamp(a, 0.0, 1.0));
    ...
    const SIMF = 196;                                    // floats in Sim (+ modes)
    ...
    const mkStepBG = (pin, vin, pout, vout) => device.createBindGroup({
    ...
    const mkDrawBG = (pcur, vcur) => device.createBindGroup({
      layout: birdPipe.getBindGroupLayout(0),
      entries: [
    ...
    const sets = [
      { grid: mkGridBG(posA), step: mkStepBG(posA, velA, posB, velB), draw: mkDrawBG(posB, velB) },
      { grid: mkGridBG(posB), step: mkStepBG(posB, velB, posA, velA), draw: mkDrawBG(posA, velA) }
    ];
    let flip = 0;
    ...
    const flock = {
      ...
      vertsPerAgent: opts.verts || 21,     // bird = 21 · fish = 33
      seed: seedBirds,
      ...
      attachScene(scene) {
        this._scene = scene;
        this._sceneDraw = [
          device.createBindGroup({ layout: birdPipe.getBindGroupLayout(0), entries: [
            { binding: 0, resource: { buffer: scene.viewBuf } }, { binding: 1, resource: { buffer: posB } }, { binding: 2, resource: { buffer: velB } }] }),
          device.createBindGroup({ layout: birdPipe.getBindGroupLayout(0), entries: [
            { binding: 0, resource: { buffer: scene.viewBuf } }, { binding: 1, resource: { buffer: posA } }, { binding: 2, resource: { buffer: velA } }] })
        ];
